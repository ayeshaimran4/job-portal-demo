...
const MyJobs = () => {
  ...
  //set current page (next -previous)
  ...
  useEffect(() => {
    setIsLoading(true)
    fetch(`http://localhost:3000/myJobs/ayeshaimransh@gmail.com`).then(res => res.json()).then(data => {
      setJobs(data);
      setIsLoading(false);
    });
  }, [searchText]);
    ...
    <div className="block w-full overflow-x-auto">
      <table className="items-center bg-transparent w-full border-collapse ">
        <thead>
          ...
          <th className="px-6 bg-blueGray-50 text-blueGray-500 align-middle border border-solid border-blueGray-100 py-3 
...

        {
          ...
          </div>) : (<tbody>
          {
            currentJobs.map((job, index) => (
              ...
              <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                <button onClick={() => handleDelete(job._id)} className="bg-red-800 py-2 px-6 text-white rounded-sm">Delete</button>
              </td>
            </tr>
            ))
          }
